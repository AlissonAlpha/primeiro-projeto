...
import { FolderOpen, Folder, Image as ImageIcon, ArrowLeft, Download, ExternalLink, RefreshCw, ChevronRight } from "lucide-react";
        ...
        <div className="px-6 py-3 border-b border-border/40 flex items-center gap-2">
          {breadcrumbs.length > 0 && (
            <button onClick={() => {
              ...
              if (prev) { navigateToBreadcrumb(breadcrumbs.length - 2); }
              else goRoot();
          ...
          <div className="flex items-center gap-1 text-sm">
            <button onClick={goRoot} className="text-muted-foreground hover:text-foreground transition-colors">
              creatives
            </button>
              ...
              {data?.sub_folders && data.sub_folders.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                    {data.sub_folders.map(f => (
                      <button key={f.path} onClick={() => navigateTo(f.path, f.name)}
                        className="flex flex-col items-center gap-2 p-4 rounded-xl border border-border/40 hover:border-violet-500/40 hover:bg-violet-500/5 transition-all group text-center">
              ...
              {data?.files && data.files.length > 0 && (
                <div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                    {data.files.map(file => (
                      <button key={file.path} onClick={() => setSelected(file)}
                        className={`relative group rounded-xl overflow-hidden border-2 transition-all aspect-square ${selected?.path === file.path ? "border-violet-500" : "border-transparent hover:border-violet-500/40"}`}>
                        {isImage(file.name) ? (
      ...
      {selected && (
        <div className="w-72 border-l border-border/50 flex flex-col bg-card/30">
          <div className="px-4 py-3 border-b border-border/40 flex items-center justify-between">
            ...
            <button onClick={() => setSelected(null)} className="text-muted-foreground hover:text-foreground transition-colors text-lg leading-none">×</button>
          </div>
          <div className="flex-1 overflow-y-auto p-4 space-y-4">
            ...
            <div className="space-y-2">
              ...
              <div>
                ...
                <p className="text-xs text-muted-foreground font-mono break-all">{selected.path.replace(selected.name, "")}</p>
              </div>
              {selected.size > 0 && (
                <div>
              ...
              {selected.created_at && (
                <div>
                  ...
                  <p className="text-sm">{new Date(selected.created_at).toLocaleDateString("pt-BR")}</p>
                </div>
              )}
            </div>
